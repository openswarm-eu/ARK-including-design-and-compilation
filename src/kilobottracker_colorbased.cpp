...
void KilobotTracker::SETUPfindKilobots()
{
    ...
    Mat res2;
    Mat display,RGBdisplay,HSVdisplay;
    vector<Mat> RGBchans;
#ifdef USE_CUDA

    RGBchans.resize(3,Mat());
    this->finalImageR.download(RGBchans[0]);
    this->finalImageG.download(RGBchans[1]);
    this->finalImageB.download(RGBchans[2]);

    merge(RGBchans,RGBdisplay);
    cvtColor(RGBdisplay,HSVdisplay,CV_RGB2HSV);
    inRange(HSVdisplay, cv::Scalar(HueMin, SaturationMin, ValueMin), cv::Scalar(HueMax, SaturationMax, ValueMax), res2);

    this->finalImageB.download(display);
     //res2.copyTo(display);//Uncomment to debug HSV thresholding
...
void KilobotTracker::trackKilobots()
{
    ...
    case CIRCLES_NAIVE:
            ...
            this->hough->detect(res2gpu,circlesGpu,stream);
